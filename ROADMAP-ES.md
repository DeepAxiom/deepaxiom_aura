...
| **Un frente web para el registry público** | `registry.deepaxiom.com` está arriba y vacío. Publicar e instalar funcionan desde la CLI, pero no hay dónde explorar lo que existe, leer un manifiesto antes de instalar, ni encontrar quién construyó un skill. | Se está construyendo como [DeepAxiom Studio](https://github.com/DeepAxiom/deepaxiom_studio) en `studio.deepaxiom.com`: React y Go en el VPS del registry, fuera de su frontera de confianza — lee el registry solo por la API pública `r1`, nunca por disco. Explorar, descargar, publicar y un directorio de desarrolladores. |
