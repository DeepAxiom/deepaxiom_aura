...
- `std/image-study@1` — `{ "frames": [{ "mime": string, "bytes_b64": string, "instance_uid": string?, "series_uid": string?, "frame": int? }], "deidentified": bool, "modality": string?, "body_part": string?, "description": string?, "report": string?, "question": string?, "prior": string? }` (imaging, e.g. `deepaxiom/cognitive/imaging-read`)
- `std/imaging-finding@1` — `{ "findings": [{ "label": string, "instance_uid": string, "box": { "x": num, "y": num, "w": num, "h": num }, "note": string?, "frame": int?, "confidence": num? }], "limitations": string, "correlation": string?, "impression": string? }` (imaging)

**`image-study@1` carries its pixels; `media-asset@1` does not.** The two
look like they should agree and they must not. A video is named by an address
because an hour of it is gigabytes; a handful of rendered frames travels inside
the envelope because they are a person's imaging, and an address would mean
whoever fetches them holds a credential to the archive that holds everybody
else's. Same subsystem, opposite answer, and the reason is whose data it is.
