...
| **Not there yet** | No multi-device view of one live session · **no failover if the node dies** — one process, and the live routing state goes with it · TEE evidence reaches `bound`, never `verified` — vendor chain verification is declared and refused rather than stubbed · the SDKs are packaged but unpublished |
