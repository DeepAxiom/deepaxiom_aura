...
class TestParseTestDecodingLine(unittest.TestCase):
    def test_insert(self):
        got = parser.parse_test_decoding_line(
    ...
    def test_delete_carries_only_replica_identity_columns(self):
        got = parser.parse_test_decoding_line("table public.t: DELETE: id[integer]:2")
    ...
    def test_begin_is_none(self):
        self.assertIsNone(parser.parse_test_decoding_line("BEGIN 733"))

    def test_commit_is_none(self):
        self.assertIsNone(parser.parse_test_decoding_line("COMMIT 733"))

    def test_empty_line_is_none(self):
        self.assertIsNone(parser.parse_test_decoding_line(""))
        self.assertIsNone(parser.parse_test_decoding_line("   "))

    # --- adversarial: malformed input must never raise ----------------------

    def test_complete_garbage_is_none_not_an_exception(self):
        self.assertIsNone(parser.parse_test_decoding_line("complete garbage, no structure at all"))
    ...
    def test_unknown_operation_is_none(self):
        ...
        self.assertIsNone(parser.parse_test_decoding_line("table public.t: TRUNCATE: "))
...
class TestFormatLSN(unittest.TestCase):
    def test_round_trips_through_postgres_notation(self):
        ...
        # replication stream.
        s = parser._format_lsn(22410856)
        hi, lo = s.split("/")
        self.assertEqual((int(hi, 16) << 32) | int(lo, 16), 22410856)

    def test_matches_a_known_value(self):
        self.assertEqual(parser._format_lsn(22410856), "0/0155F668")
